...
func TestTerminalReportStoreRoundTripAndPermissions(t *testing.T) {
	...
	report := terminalTaskReport{
		...
		taskDispatchedAt:      "2026-09-18T10:15:00Z",
		skillExecutionManifest: &skillbundle.ExecutionManifest{
			Version: skillbundle.ExecutionManifestVersion,
			Skills:  []skillbundle.ExecutionRecord{{Source: "workspace", SkillID: "skill-1", BundleHash: strings.Repeat("a", 64)}},
		},
	}
	...
	if err := store.enqueue(report); err != nil {
		t.Fatalf("re-enqueue identical manifest after decoding: %v", err)
	}
