...
func buildResolvedSkillExecutionManifest(skills []SkillData) *skillbundle.ExecutionManifest {
	...
	resolved := make([]skillbundle.ResolvedSkill, 0, len(skills))
	for _, s := range skills {
		files := make([]skillbundle.File, 0, len(s.Files))
		for _, f := range s.Files {
			files = append(files, skillbundle.File{Path: f.Path, Content: f.Content})
		}
		bundle := skillbundle.Skill{ID: s.ID, Source: s.Source, Name: s.Name, Description: s.Description, Content: s.Content, Files: files}
		resolved = append(resolved, skillbundle.ResolvedSkill{Bundle: bundle, DeclaredHash: s.Hash})
	}
	manifest, err := skillbundle.BuildExecutionManifest(resolved)
	if err != nil {
		return nil
	}
	return &manifest
...
func (d *Daemon) sendTerminalTaskReport(ctx context.Context, report terminalTaskReport, schedule []time.Duration) error {
	...
	case terminalTaskReportComplete:
		return d.client.completeTaskWithRetrySchedule(ctx, report.taskID, report.output, report.branchName, report.sessionID, report.workDir, report.sessionRolloutMissing, report.retiredSessionID, report.durableWorkDir, report.taskDispatchedAt, report.skillExecutionManifest, schedule)
