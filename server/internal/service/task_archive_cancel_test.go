...
func newArchiveCancelFixture(t *testing.T, pool *pgxpool.Pool) (*testutil.Fixture, string, string) {
	t.Helper()
...
func TestCancelTasksForArchivedAgentPublishesCommittedChatCancellations(t *testing.T) {
	...
	fx, agentID, runtimeID := newArchiveCancelFixture(t, pool)
...
func TestCancelTasksForArchivedAgentCommitFailureDoesNotPublish(t *testing.T) {
	...
	starter := &archiveCancelRejectCommit{pool: pool, taskID: taskID, err: injectedErr}
	bus := events.New()
	var published int
	bus.Subscribe(protocol.EventTaskCancelled, func(events.Event) { published++ })
	svc := NewTaskService(db.New(pool), starter, nil, bus)
	...
	if err := fx.DB.QueryRow(ctx, `SELECT status FROM agent_task_queue WHERE id = $1`, taskID).Scan(&persistedStatus); err != nil {
