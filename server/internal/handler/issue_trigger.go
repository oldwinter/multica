...
func (h *Handler) dispatchIssueRun(ctx context.Context, issue db.Issue, trigger service.IssueRunTrigger, actorType, actorID, handoffNote string, twinUse *service.TwinOneOffUsePolicyOverride) {
	...
	case "agent":
		...
		_, _ = h.TaskService.EnqueueTaskForIssueWithHandoffAndTwin(ctx, issue, handoffNote, memberActorUserID(actorType, actorID), twinUse)
	case "squad":
		h.enqueueSquadLeaderTask(ctx, issue, pgtype.UUID{}, actorType, actorID, handoffNote, twinUse)
	}
}
