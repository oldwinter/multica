	...
	for _, reader := range []struct {
		...
		for _, suffix := range []string{"", "?since=2"} {
			t.Run(reader.name+suffix, func(t *testing.T) {
				...
				ctx := req.Context()
				if reader.name == "user" {
					// The fixture agent is private: regular transcript reads must
					// carry the owner's authenticated identity, as the router does.
					req.Header.Set("X-User-ID", testUserID)
					ctx = middleware.SetMemberContext(ctx, testWorkspaceID, member)
				} else {
					ctx = middleware.WithDaemonContext(ctx, testWorkspaceID, "call-id-daemon")
				}
