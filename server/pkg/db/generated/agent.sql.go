...
WHERE id = (
    ...
    WHERE atq.agent_id = $2 AND atq.status = 'queued'
...
func (q *Queries) ClaimAgentTask(ctx context.Context, arg ClaimAgentTaskParams) (AgentTaskQueue, error) {
	row := q.db.QueryRow(ctx, claimAgentTask, arg.PrepareLeaseSecs, arg.AgentID, arg.IncludeRoomTasks)
	var i AgentTaskQueue
	err := row.Scan(
		&i.ID,
		&i.AgentID,
