...
WHERE id = (
    ...
    WHERE atq.agent_id = $2
      ...
      AND ($4::boolean OR atq.room_turn_id IS NULL)
      AND EXISTS (
          SELECT 1
          FROM agent a
...
type ClaimAgentTaskParams struct {
	...
	AgentID          pgtype.UUID `json:"agent_id"`
	RuntimeID        pgtype.UUID `json:"runtime_id"`
	IncludeRoomTasks bool        `json:"include_room_tasks"`
...
SELECT
    ...
    p.chat_input_task_id, $2, p.room_turn_id,
...
WHERE atq.runtime_id = $1
  AND atq.status = 'queued'
  AND ($2::boolean OR atq.room_turn_id IS NULL)
  AND EXISTS (
...
func (q *Queries) ListQueuedClaimCandidatesByRuntime(ctx context.Context, arg ListQueuedClaimCandidatesByRuntimeParams) ([]AgentTaskQueue, error) {
	rows, err := q.db.Query(ctx, listQueuedClaimCandidatesByRuntime, arg.RuntimeID, arg.IncludeRoomTasks)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (q *Queries) ListQueuedClaimCandidatesByRuntimes(ctx context.Context, arg ListQueuedClaimCandidatesByRuntimesParams) ([]AgentTaskQueue, error) {
	rows, err := q.db.Query(ctx, listQueuedClaimCandidatesByRuntimes, arg.RuntimeIds, arg.IncludeRoomTasks)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
WHERE id = (
    ...
    WHERE atq.runtime_id = $2
      ...
      AND ($3::boolean OR atq.room_turn_id IS NULL)
