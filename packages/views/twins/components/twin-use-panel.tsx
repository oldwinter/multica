...
const LONG_LIVED_SCOPES = ["workspace", "agent", "project", "issue"] as const satisfies readonly TwinBindingScope[];
const BINDING_STATES = ["off", "preview", "enabled"] as const satisfies readonly TwinBindingState[];
...
export function TwinUsePanel({ wsId, versions, currentVersionId, canManage, onGuide }: {
  ...
  const stateDescriptions: Record<TwinBindingState, string> = {
    off: t(($) => $.use.state_off_description),
    preview: t(($) => $.use.state_preview_description),
    enabled: t(($) => $.use.state_enabled_description),
  };
  const stateDescription = stateDescriptions[state] ?? stateDescriptions.off;
  const stateOptions = useMemo(() => BINDING_STATES.map((value) => [value, bindingStateLabel(value, t)] as const), [t]);
  const scopeLabels: Partial<Record<TwinBindingScope, string>> = {
    workspace: t(($) => $.use.scope_workspace),
    agent: t(($) => $.use.scope_agent),
    project: t(($) => $.use.scope_project),
    issue: t(($) => $.use.scope_issue),
  };
  const scopeLabel = (value: TwinBindingScope) => scopeLabels[value] ?? value;
    ...
    <div className="space-y-7" data-testid="twin-use-panel">
      ...
      <TwinDestination destination="use-binding" className="space-y-4 border-y border-border/70 py-5" aria-labelledby="twin-binding-title">
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          <label className="grid min-w-0 gap-2 text-label font-medium">{t(($) => $.use.scope)}<Select items={scopeOptions} value={scopeType} onValueChange={(value) => value && setScopeType(value)}><SelectTrigger className="w-full min-w-0 focus-visible:ring-2 focus-visible:ring-ring" aria-label={t(($) => $.use.scope)} disabled={!mayManage}><SelectValue /></SelectTrigger><SelectContent>{scopeOptions.map((item) => <SelectItem key={item.value} value={item.value}>{item.label}</SelectItem>)}</SelectContent></Select></label>
          ...
          <label className="grid min-w-0 gap-2 text-label font-medium">{t(($) => $.use.version)}<Select items={versionOptions} value={versionId} onValueChange={(value) => value && setVersionId(value)}><SelectTrigger className="w-full min-w-0 focus-visible:ring-2 focus-visible:ring-ring" aria-label={t(($) => $.use.version)} disabled={!mayManage || versionOptions.length === 0}><SelectValue /></SelectTrigger><SelectContent>{versionOptions.map((item) => <SelectItem key={item.value} value={item.value}>{item.label}</SelectItem>)}</SelectContent></Select></label>
        </div>
        <fieldset className="grid gap-2 disabled:opacity-60" disabled={!mayManage}><legend className="text-label font-medium">{t(($) => $.use.state)}</legend><SegmentedToggle value={state} options={stateOptions} onChange={setState} buttonClassName="min-h-9 px-3 py-2 text-body" /><p className="max-w-prose text-caption text-muted-foreground" aria-live="polite">{stateDescription}</p></fieldset>
        {mutationError ? <p role="alert" className="text-body text-destructive">{t(($) => $.use.mutation_failed)}</p> : null}
        <Button className="w-full sm:w-auto" disabled={!mayManage || !effectiveScopeId || !versionId || upsertBinding.isPending} onClick={() => upsertBinding.mutate({ scopeType, scopeId: effectiveScopeId, state, twinVersionId: versionId })}><Save data-icon="inline-start" />{upsertBinding.isPending ? t(($) => $.use.saving_binding) : t(($) => $.use.save_binding)}</Button>
      </TwinDestination>

      <section className="space-y-3" aria-labelledby="configured-bindings-title"><h3 id="configured-bindings-title" className="text-title font-medium text-foreground">{t(($) => $.use.configured_title)}</h3>{bindings.length === 0 ? <p className="text-body text-muted-foreground">{t(($) => $.use.configured_empty)}</p> : <div className="divide-y divide-border/70">{bindings.map((binding) => <div key={binding.id} className="flex min-w-0 flex-wrap items-center gap-2 py-3 first:pt-0 last:pb-0 sm:flex-nowrap sm:gap-3"><Badge variant="outline" className="shrink-0">{scopeLabel(binding.scopeType)}</Badge><span className="min-w-0 flex-1 truncate text-body text-foreground">{entityLabel(binding)}</span><span className="text-caption text-muted-foreground">{versionsById.get(binding.twinVersionId) ?? t(($) => $.use.unavailable_version)}</span><Badge variant={binding.state === "enabled" ? "default" : "secondary"} className="shrink-0">{bindingStateLabel(binding.state, t)}</Badge>{mayManage ? <Tooltip><TooltipTrigger render={<Button variant="ghost" size="icon-sm" aria-label={t(($) => $.use.delete_binding)} />} onClick={() => { deleteBinding.reset(); setDeleteBindingId(binding.id); }} disabled={deleteBinding.isPending}><Trash2 aria-hidden="true" /></TooltipTrigger><TooltipContent>{t(($) => $.use.delete_binding)}</TooltipContent></Tooltip> : null}</div>)}</div>}</section>
      ...
      <TwinDestination destination="use-preview" className="space-y-4 border-y border-border/70 py-5" aria-labelledby="twin-preview-title">
        ...
        <Button variant="outline" className="w-full sm:w-auto" disabled={!previewAgent || !request.trim() || previewBriefing.isPending} onClick={() => {
