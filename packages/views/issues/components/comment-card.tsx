        ...
        <div data-comment-actions className="ml-auto flex shrink-0 items-center gap-0.5">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              {onCopyLink && <DropdownMenuItem onClick={() => onCopyLink(entry.id)}>
  ...
  const renderAnchoredRuns = (commentId: string) => runs.filter((run) => run.anchorCommentId === commentId
    ...
    .map((run) => {
      ...
      return <Fragment key={run.task.id}><AgentRunComment run={run} entering={enteringRunIds?.has(run.task.id)} commentProps={reply ? {
         issueId, issueHref, entry: reply, replies: [], currentUserId, canModerate, onReply, onEdit, onDelete,
         onToggleReaction, onCreateSubIssue, onResolveToggle, onCopyLink: copyCommentLink, onQuote: quoteInReply,
         highlightedCommentId, enteringRunIds,
