...
function renderDialog(
  ...
  options: {
    ...
    dialogProps?: Partial<AgentTranscriptDialogProps>;
  } = {},
) {
...
describe("AgentTranscriptDialog — task run review", () => {
  const reviewSkills = [
  ...
  it("submits helpful feedback with a concrete reason", async () => {
    const onTaskRunReview = vi.fn().mockResolvedValue({ id: "review-1" });
    renderDialog(items, {
      dialogProps: { onTaskRunReview, taskRunReviewSkills: reviewSkills },
    });
    ...
    await userEvent.type(
      ...
      "The cited source made the answer verifiable.",
    );
    await userEvent.click(screen.getByRole("button", { name: "Submit review" }));

    expect(onTaskRunReview).toHaveBeenCalledWith({
      outcome: "helpful",
      target: "knowledge",
  ...
  it("requires a correction and a listed workspace skill for skill procedure feedback", async () => {
    const onTaskRunReview = vi.fn().mockResolvedValue({ id: "review-1" });
    renderDialog(items, {
      dialogProps: { onTaskRunReview, taskRunReviewSkills: reviewSkills },
    });
    ...
    await userEvent.type(
      screen.getByLabelText("Reason"),
      "The run skipped a required compatibility check.",
    );
    await userEvent.click(screen.getByRole("button", { name: "Submit review" }));

    expect(onTaskRunReview).not.toHaveBeenCalled();
    ...
    await userEvent.type(
      screen.getByLabelText("Specific correction"),
      "Keep the compatibility fallback and cover it with a regression test.",
    );
    await userEvent.click(screen.getByRole("button", { name: "Submit review" }));

    expect(onTaskRunReview).toHaveBeenCalledWith(expect.objectContaining({
      outcome: "needs_correction",
      target: "skill_procedure",
      skillId: "skill-1",
      correction: "Keep the compatibility fallback and cover it with a regression test.",
    }));
    expect(screen.queryByRole("textbox", { name: /Skill ID/i })).not.toBeInTheDocument();
  });

  it("keeps the same idempotency key when an unavailable endpoint is retried", async () => {
    const onTaskRunReview = vi.fn().mockRejectedValue(new Error("not found"));
    renderDialog(items, {
      dialogProps: {
        onTaskRunReview,
        taskRunReviewError: true,
        taskRunReviewSkills: reviewSkills,
      },
    });
    ...
    await userEvent.type(
      screen.getByLabelText("Reason"),
      "The run exposed a reproducible crash in the product.",
    );
    await userEvent.click(screen.getByRole("button", { name: "Try again" }));
    await userEvent.click(screen.getByRole("button", { name: "Try again" }));

    expect(screen.getByText(/transcript is still available/)).toBeInTheDocument();
    expect(screen.getByText(/Agent summary/)).toBeInTheDocument();
    expect(onTaskRunReview).toHaveBeenCalledTimes(2);
    expect(onTaskRunReview.mock.calls[0]?.[0].idempotencyKey)
      .toBe(onTaskRunReview.mock.calls[1]?.[0].idempotencyKey);
  });

  it("returns focus to the review action on Escape and focuses confirmation", async () => {
    const props = {
      onTaskRunReview: vi.fn().mockResolvedValue({ id: "review-1" }),
      taskRunReviewSkills: reviewSkills,
    };
    const rendered = renderDialog(items, { dialogProps: props });

    await userEvent.click(screen.getByRole("button", { name: "Review this run" }));
    await userEvent.keyboard("{Escape}");
    await waitFor(() => {
      expect(screen.getByRole("button", { name: "Review this run" })).toHaveFocus();
    });
  ...
  it("keeps the review UI off live runs and surfaces without review permission", () => {
    const onTaskRunReview = vi.fn().mockResolvedValue({ id: "review-1" });
    const { rerender } = renderDialog(items, {
      task: liveTask,
      isLive: true,
      dialogProps: { onTaskRunReview },
    });
