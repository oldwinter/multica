...
일부 커뮤니티 또는 다운스트림 macOS 패키지는 Developer ID 서명과 Apple 공증이 없습니다. 신뢰하는 GitHub 저장소에서 설치 패키지를 내려받은 경우에만 경고를 직접 해제하세요.
</Callout>

### macOS: 서명되지 않은 빌드 열기

macOS에서 개발자를 확인할 수 없거나 앱에 악성 소프트웨어가 있는지 확인할 수 없다고 표시되는 경우:

1. Multica를 한 번 열어 본 뒤 경고를 닫습니다.
2. **Apple 메뉴 → 시스템 설정 → 개인정보 보호 및 보안**을 열고 **보안**까지 아래로 스크롤합니다.
3. 차단된 Multica 옆의 **확인 없이 열기**를 클릭하고 인증한 다음 **열기**를 다시 확인합니다.

macOS는 해당 Multica에만 예외를 기록하므로 다음부터는 정상적으로 실행할 수 있습니다. 이 버튼은 앱이 한 번 차단된 뒤에만 나타나며 제한된 시간 동안만 표시될 수 있습니다. Gatekeeper를 시스템 전체에서 비활성화하지 마세요. 화면 문구는 [Apple의 최신 안내](https://support.apple.com/102445)를 확인하세요.
...
- 서명되지 않은 커뮤니티 또는 다운스트림 macOS 앱은 자동 업데이트할 수 없습니다. 새 `.dmg`를 매번 직접 설치하세요.
