...
コミュニティ版やダウンストリーム版の macOS パッケージには、Developer ID 署名と Apple の公証がない場合があります。信頼できる GitHub リポジトリからダウンロードした場合に限って、警告を手動で解除してください。
</Callout>

### macOS：未署名ビルドを開く

macOS に開発元を確認できない、または悪意のあるソフトウェアを確認できないと表示された場合：

1. Multica を一度開こうとして、警告を閉じます。
2. **Apple メニュー → システム設定 → プライバシーとセキュリティ**を開き、**セキュリティ**までスクロールします。
3. ブロックされた Multica の横にある**このまま開く**をクリックし、認証してからもう一度**開く**を確認します。

macOS はその Multica にだけ例外を記録するため、次回から通常どおり起動できます。ボタンは一度ブロックされた後にだけ表示され、表示時間にも限りがあります。Gatekeeper をシステム全体で無効にしないでください。画面の表記は [Apple の最新手順](https://support.apple.com/102445)を確認してください。
...
- 未署名のコミュニティ版またはダウンストリーム版 macOS アプリは自動更新できません。新しい `.dmg` を毎回手動でインストールしてください。
