...
이 레이어를 사용하는 기능은 세 가지입니다. 앞의 두 기능은 아래와 같이 채팅 내용을 전송하고, Twin Build는 명시적인 작업 후에만 검토된 근거를 전송합니다.
...
API key와 base URL이 모두 비어 있으면 이 레이어가 꺼지고 업스트림 요청을 전혀 보내지 않습니다. 이 레이어가 채팅 또는 Twin 근거를 배포 환경 밖으로 보내면 안 되는 정책이라면 이것이 지원되는 구성입니다. 세션은 클라이언트가 첫 메시지에서 만든 제목을 그대로 사용하고, 후속 질문 버튼은 표시되지 않으며, Twin Build는 모델 생성을 사용할 수 없다고 명확히 표시합니다. 에이전트 task 실행은 별도의 경로이며 영향을 받지 않습니다.
