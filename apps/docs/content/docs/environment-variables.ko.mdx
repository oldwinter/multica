...
이 레이어를 사용하는 기능은 네 가지입니다. 앞의 두 기능은 아래와 같이 채팅 내용을 전송하고, Twin Build와 Skill replay는 승인된 근거를 전송합니다.
...
- **Twin Build** — 워크스페이스 owner 또는 admin이 명시적으로 Build를 실행한 경우에만 승인된 변경 불가능 LM Wiki canonical JSON, 안전한 citation key, 이전에 서명된 assertion을 설정된 endpoint로 전송합니다. 변경 가능한 Workspace Wiki나 개인 Wiki 콘텐츠, local-only 근거, 원시 경로 또는 basename, 비공개 프로필 이름, 자격 증명은 전송하지 않습니다.
- **Skill evolution replay** — 현재 승인되고 크기가 제한된 근거 case 하나를 base Skill 및 candidate Skill과 함께 실행하고 비교하도록 전송합니다. case와 모델 응답은 호출 중에만 유지되며, 본문이 없는 판정, 개수, digest, 소요 시간, cost만 저장됩니다.

API key와 base URL이 모두 비어 있으면 이 레이어가 꺼지고 업스트림 요청을 전혀 보내지 않습니다. 이 레이어가 채팅, Twin 또는 Skill replay 근거를 배포 환경 밖으로 보내면 안 되는 정책이라면 이것이 지원되는 구성입니다. 세션은 클라이언트가 첫 메시지에서 만든 제목을 그대로 사용하고, 후속 질문 버튼은 표시되지 않으며, Twin Build와 Skill replay는 모델 생성을 사용할 수 없다고 명확히 표시합니다. 에이전트 task 실행은 별도의 경로이며 영향을 받지 않습니다.
