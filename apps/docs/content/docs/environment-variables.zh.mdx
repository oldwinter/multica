...
有三个功能会使用这一层。前两个功能会按下述方式发送聊天内容；Twin 构建只会在用户明确操作后发送已审核的证据：
...
API key 与 base URL 都为空时，这一层关闭，不会发出任何上游请求。如果你的政策不允许这一层把聊天或 Twin 证据发到部署之外，这就是受支持的配置方式：对话继续使用客户端根据第一条消息生成的标题，后续提问按钮不再出现，Twin Build 会明确显示模型生成不可用。智能体的 task 执行走独立路径，不受影响。
