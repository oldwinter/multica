...
有四个功能会使用这一层。前两个功能会按下述方式发送聊天内容；Twin 构建和 skill 回放会发送经授权的证据：

- **对话标题自动生成** —— 发送新对话中用户的第一条消息，原文发送。附件不会包含在内。
- **后续提问建议**（智能体回复下方的按钮）—— 发送对话末尾的内容：最多 6 条消息，其中被追问的那条回复上限 3000 字符，更早的每条上限 800 字符。
- **Twin 构建（Build）** —— 只有工作区 owner 或 admin 明确执行 Build 时，才会把已接受且不可变的 LM Wiki 规范 JSON、安全的引用键和上一版本已签署的断言发送到配置的 endpoint。它绝不会发送可变的工作区 Wiki 内容、个人 Wiki 内容、local-only 证据、原始路径或 basename、私有配置档名称或凭据。
- **Skill 演进回放** —— 将一条当前已授权且有大小上限的证据 case 连同基础 skill 和候选 skill 发送出去执行并比较。case 与模型响应只在调用期间存在；持久化内容仅包含无正文的结论、计数、摘要、耗时和成本。

API key 与 base URL 都为空时，这一层关闭，不会发出任何上游请求。如果你的政策不允许这一层把聊天、Twin 或 skill 回放证据发到部署之外，这就是受支持的配置方式：对话继续使用客户端根据第一条消息生成的标题，后续提问按钮不再出现，Twin Build 和 skill 回放会明确显示模型生成不可用。智能体的 task 执行走独立路径，不受影响。
