...
Some community and downstream macOS releases are unsigned and have not been notarized by Apple. Only override the warning when you downloaded the installer from the GitHub repository you trust.
</Callout>

### macOS: opening an unsigned build

If macOS says it cannot verify the developer or check the app for malicious software:

1. Try to open Multica once, then close the warning.
2. Open **Apple menu → System Settings → Privacy & Security** and scroll to **Security**.
3. Click **Open Anyway** next to the blocked Multica app, authenticate, then confirm **Open**.

macOS records an exception for that copy of Multica, so later launches work normally. The button only appears after a blocked launch and may be available for a limited time. Do not disable Gatekeeper globally. See [Apple's current instructions](https://support.apple.com/102445) for the exact system wording.
...
- unsigned community or downstream macOS builds cannot auto-update; install each newer `.dmg` manually;
