...
Three features use this layer. The first two send chat content as described below; Twin Build sends reviewed evidence only after an explicit action:
...
When both the API key and the base URL are empty, this layer is disabled and makes no upstream request at all. That is the supported configuration when your policy does not allow this layer to send chat or Twin evidence off the deployment: chat sessions keep the title the client derives from the first message, follow-up question buttons do not appear, and Twin Build clearly reports that model generation is unavailable. Agent task execution is a separate path and remains unaffected.
