...
Four features use this layer. The first two send chat content as described below; Twin Build and Skill replay send authorized evidence:
...
- **Twin Build** — only an explicit Build action by a workspace owner or admin sends the accepted, immutable LM Wiki canonical JSON, safe citation keys, and prior signed assertions to the configured endpoint. It never sends mutable Workspace Wiki content, personal Wiki content, local-only evidence, raw paths or basenames, private profile names, or credentials.
- **Skill evolution replay** — sends one currently authorized, size-bounded evidence case with the base and candidate Skill for execution and comparison. The case and model responses are transient; only content-free verdicts, counts, digests, duration, and cost are persisted.

When both the API key and the base URL are empty, this layer is disabled and makes no upstream request at all. That is the supported configuration when your policy does not allow this layer to send chat, Twin, or Skill replay evidence off the deployment: chat sessions keep the title the client derives from the first message, follow-up question buttons do not appear, and Twin Build and Skill replay report that model generation is unavailable. Agent task execution is a separate path and remains unaffected.
