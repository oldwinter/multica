...
このレイヤーを使う機能は 3 つあります。最初の 2 つは以下のとおりチャット内容を送信し、Twin Build は明示的な操作の後にのみレビュー済みの証拠を送信します。
...
API key と base URL の両方が空の場合、このレイヤーは無効になり、上流へのリクエストを一切行いません。このレイヤーからチャットや Twin の証拠をデプロイ外へ出せないポリシーでは、これがサポートされた構成です。セッションはクライアントが最初のメッセージから生成したタイトルをそのまま使い、フォローアップ質問のボタンは表示されず、Twin Build はモデル生成を利用できないことを明示します。エージェントの task 実行は別の経路であり、影響を受けません。
