...
このレイヤーを使う機能は 4 つあります。最初の 2 つは以下のとおりチャット内容を送信し、Twin Build と Skill リプレイは承認済みの証拠を送信します。
...
- **Twin Build** — ワークスペースの owner または admin が明示的に Build を実行した場合に限り、承認済みでイミュータブルな LM Wiki の canonical JSON、安全な citation key、直前の署名済み assertion を設定済み endpoint に送信します。可変の Workspace Wiki や個人 Wiki の内容、local-only の証拠、生のパスや basename、非公開のプロファイル名、資格情報は送信しません。
- **Skill evolution リプレイ** — 現在承認され、サイズ制限された 1 件の証拠 case を、base Skill および candidate Skill とともに実行と比較のため送信します。case とモデル応答は呼び出し中のみ保持され、本文を含まない判定、件数、digest、所要時間、cost だけが永続化されます。

API key と base URL の両方が空の場合、このレイヤーは無効になり、上流へのリクエストを一切行いません。このレイヤーからチャット、Twin、Skill リプレイの証拠をデプロイ外へ出せないポリシーでは、これがサポートされた構成です。セッションはクライアントが最初のメッセージから生成したタイトルをそのまま使い、フォローアップ質問のボタンは表示されず、Twin Build と Skill リプレイはモデル生成を利用できないことを明示します。エージェントの task 実行は別の経路であり、影響を受けません。
