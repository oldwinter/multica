...
export function DocsHero({
  ...
  return (
    <section className="not-prose mb-7 pt-2">
      {eyebrow ? (
        <p className="mb-5 text-[0.6875rem] font-semibold uppercase tracking-normal text-muted-foreground">
          {eyebrow}
        </p>
      ) : null}
      <h1
        className={`${subtitle ? "mb-5" : "mb-0"} font-[family-name:var(--font-serif)] text-[2.25rem] font-normal leading-[1.05] tracking-normal text-foreground sm:text-[2.75rem]`}
      >
        {title}
      </h1>
      {subtitle ? (
        <p className="max-w-[36rem] font-[family-name:var(--font-serif)] text-[1.25rem] leading-[1.5] tracking-normal text-[oklch(from_var(--foreground)_calc(l+0.06)_c_h)]">
          {subtitle}
        </p>
      ) : null}
    </section>
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-1.5">
        <span className="font-[family-name:var(--font-serif)] text-[1.0625rem] font-medium tracking-normal text-foreground">
          {title}
        </span>
